...
import { ToggleEmployeeActiveService } from "@/features/employees/toggle-employee-active.service";
...
interface ToggleEmployeeActiveModalProps {
  employee: UserClientModel;
  onClose: () => void;
}

export const ToggleEmployeeActiveModal = ({
  employee,
  onClose,
}: ToggleEmployeeActiveModalProps) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await ToggleEmployeeActiveService(employee.id, password.trim());
      ...
      // Other failures (e.g. the 0-invoice/restock-records requirement) are
      // already surfaced as a toast by the service — keep the modal open so
      // the user can see it and adjust/cancel.
    } finally {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border shadow-lg p-8 flex flex-col gap-4 justify-center items-center relative">
        ...
        <div className="flex gap-3 w-full items-start">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-semibold">{actionLabel} an Employee</span>
          </div>
        </div>
