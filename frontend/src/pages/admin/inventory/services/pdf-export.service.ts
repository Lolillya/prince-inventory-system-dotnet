...
export const exportPricelistPdf = (
    ...
    const flattenedRows = source.flatMap((item) => {
    ...
    source.forEach((item) => {
        ...
        const codeLines = doc.splitTextToSize(item.product.product_Code ?? "-", codeWidth);
        const descLines = doc.splitTextToSize(buildProductDescription(item), descWidth);

        const rowContentHeight = Math.max(codeLines.length * 5, descLines.length * 5);
        ...
        levelsToRender.forEach((lvl, index) => {
            ...
            if (index === 0) {
                ...
                doc.setFontSize(9.5);
                doc.setTextColor(153, 153, 153);
                doc.text(`${lvl.unitOfMeasure.uom_Name} (${lvl.conversion_Factor}x)`, xUom + 6, currentY);
            }
...
export const exportStocklistPdf = (
    ...
    const filteredItems = inventory.filter((item) => {
        ...
        const isLow = !isNoStock && !isVeryLow && lowLevel != null && qty <= lowLevel;
    ...
    filteredItems.forEach((item) => {
        const preset = item.unitPresets[0];
        const sortedLevels = [...(preset?.preset?.presetLevels ?? [])].sort((a, b) => a.level - b.level);
        const sortedQuantities = [...(preset?.presetQuantities ?? [])].sort((a, b) => a.level - b.level);

        const levelsToRender = includePackagingHierarchy ? sortedLevels : sortedLevels.slice(0, 1);

        const codeLines = doc.splitTextToSize(item.product.product_Code ?? "-", 40);
        const descLines = doc.splitTextToSize(buildProductDescription(item), descMaxWidth);

        const textContentHeight = Math.max(codeLines.length * 5, descLines.length * 5);
        const uomBlockHeight = levelsToRender.length * 5;
    ...
    const hierarchySuffix = includePackagingHierarchy ? "with-hierarchy" : "without-hierarchy";
    doc.save(`inventory-stocklist-${filterSuffix || "all"}-${hierarchySuffix}.pdf`);
};
